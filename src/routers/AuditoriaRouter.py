    ...
    try:
        ...
        if data_inicio:
            try:
                ...
                query = query.filter(AuditoriaDB.data_hora >= dt)
            except ValueError:
                raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Data início inválida. Use YYYY-MM-DD")
        if data_fim:
            try:
                dt = datetime.strptime(data_fim, "%Y-%m-%d")
                query = query.filter(AuditoriaDB.data_hora <= dt)
            except ValueError:
                raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Data fim inválida. Use YYYY-MM-DD")
...
async def listar_acoes_disponiveis(
    ...
    try:
        ...
        return {
            "acoes": [{"codigo": a[0]} for a in acoes_db],
            "recursos": [{"codigo": r[0]} for r in recursos_db]
        }
    except Exception as e:
